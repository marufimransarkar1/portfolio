    ...
    <section id="portfolio" className="py-24 bg-surface-low px-6 md:px-12">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
          <div>
            <h3 className="text-sm text-center md:text-left uppercase tracking-[0.3em] text-secondary mb-4 font-medium">
              Showcase
            </h3>
          ...
          <p className="text-white/40 max-w-md md:text-right">
            Explore a curated selection of projects where technical precision
            meets high-end digital aesthetics.
          </p>
        </div>
          ...
          {projects.map((project, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-linear-to-t from-background via-background/20 to-transparent opacity-0 group-hover:opacity-200 transition-all duration-500 flex flex-col justify-end p-8">
                <span
                  className={`${project.color} text-xs font-bold uppercase tracking-widest mb-2`}
                >
                  {project.category}
                </span>
                <h4 className="text-2xl font-bold mb-4 font-headline">
                  {project.title}
                </h4>
                <a
                  href={project.link}
